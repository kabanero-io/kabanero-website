...
source curl -fsSL https://clis.cloud.ibm.com/install/linux | sh
...
if [ "$JEKYLL_ENV"="production" ]; then
    ...
    echo "dev build"
    echo $SCRIPT_DIR/build_clone_blogs.sh $BLOGS_GIT_URL $BLOGS_GIT_REVISION
    echo $SCRIPT_DIR/build_clone_docs.sh  "$DOCS_GIT_URL"  "$DOCS_GIT_REVISION"
